...
%位置给定的二阶导数，即加速度
ddR1 = -0.0625*sin(0.25*t);
ddR2 = -0.0625*sin(0.25*t);
ddR3 = -0.0625*sin(0.25*t);
ddR4 = -sin(t);
ddR5 = -sin(t);
ddR6 = -sin(t);
...
%细胞数组J为各个连杆的转动惯量
J{1} = [1.9627345061226189, 0, 0;
            0, 2.60480624047246, 0;
            0, 0, 1.8795078158813859];
J{2} = [1.5382657404520751, 0, 0;
            0, 1.4917350637236482, 0;
            0, 0, 0.080249418321229983];
J{3} = [0.52446557260586973, 0, 0;
            0, 0.73236553459530184, 0;
            0, 0, 0.51463961316216178];
J{4} = [-0.084654168245494366, 0, 0;
            0, 0.35548325308117125, 0;
            0, 0, 0.391710484509198];
J{5} = [0.0098966460782527956, 0, 0;
            0, 0.010025927190006987, 0;
            0, 0, 0.011728543379556848];
J{6} = [0.00113, 0, 0;
            0, 0.000592, 0;
            0, 0, 0.000578];
%创建W矩阵
...
tol = M*(ddR + c.*de + eps*(s.^2).*tanh(s/sigma) + k1.*abs(s).^(alpha).*tanh(s/sigma) + k2.*abs(s).^(beta).*tanh(s/sigma) + dc) + N*dth';
